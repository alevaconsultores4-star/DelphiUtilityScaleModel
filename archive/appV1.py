...
# Streamlit single-file app with: Projects + Scenarios, Macro, Timeline, Generation, Revenues, Comparison
...
@dataclass
class MacroInputs:
    # Annual rates in % (e.g., 6.0 means 6%)
    col_cpi: float = 6.0
    col_ppi: float = 5.0
    us_cpi: float = 3.0
    ...
    custom_index_rate: float = 6.0  # used if index = Custom

    # Optional yearly override tables (year -> rate)
...
class TimelineInputs:
    ...
    # Optional milestone placeholder (not modeled financially yet)
    bank_guarantee_month: Optional[int] = None  # months from start
...
class GenerationInputs:
    ...
    degradation_pct: float = 0.5  # % per year


@dataclass
class RevenueOption1PPA:
    ppa_price_cop_per_kwh: float = 320.0  # nominal COP at COD year start
    ppa_term_years: int = 12
    merchant_price_cop_per_kwh: float = 250.0  # nominal COP after term
    indexation: str = "Colombia CPI"  # CPI/PPI/US/Custom


def _default_manual_prices() -> Dict[int, float]:
    # default 25 years at 300 COP/kWh
    return {i: 300.0 for i in range(1, 26)}


@dataclass
class RevenueOption2Manual:
    # constant COP series (base-year) by operating year number (1..N)
...
class ScenarioInputs:
    ...
    revenue_mode: str = "Standard PPA Parameters"  # or "Manual Annual Series"
...
def operating_year_table(s: ScenarioInputs) -> pd.DataFrame:
    ...
    idx_key = {"Colombia CPI": "col_cpi", "Colombia PPI": "col_ppi", "US CPI": "us_cpi", "Custom": "custom"}[
        index_choice
    ]
...
tab_macro, tab_timeline, tab_gen, tab_rev, tab_compare = st.tabs(
    ["A) Macroeconomic", "B) Timeline", "C) Power Generation", "D) Power Revenues", "Compare"]
)
...
with tab_macro:
    ...
    with fx1:
        s.macro.fx_cop_per_usd_start = st.number_input(
            "Starting FX (COP/USD)", value=float(s.macro.fx_cop_per_usd_start), step=10.0
        )
    with fx2:
        s.macro.fx_method = st.selectbox(
            "FX method",
            ["Inflation differential (PPP approx.)", "Flat"],
            index=0 if s.macro.fx_method != "Flat" else 1,
        )
    with fx3:
        s.macro.fx_flat = st.number_input(
            "Flat FX (if selected)", value=float(s.macro.fx_flat or s.macro.fx_cop_per_usd_start), step=10.0
        )
...
with tab_timeline:
    ...
    st.caption("RTB marks the end of Development and start of CAPEX. COD marks the start of Operation and revenue generation.")
...
with tab_gen:
    ...
    with g4:
        s.generation.production_choice = st.selectbox(
            "Production choice", ["P50", "P75", "P90"], index=["P50", "P75", "P90"].index(s.generation.production_choice)
        )
...
with tab_rev:
    ...
    if mode == "Standard PPA Parameters":
        ...
        with c1:
            r.indexation = st.selectbox("Indexation", INDEX_CHOICES, index=INDEX_CHOICES.index(r.indexation) if r.indexation in INDEX_CHOICES else 0)
        with c2:
            st.caption("Enter constant COP/kWh for each operating year. Indexation will convert to nominal.")
        ...
        edited = st.data_editor(base_tbl, use_container_width=True, hide_index=True, num_rows="fixed")
        r.prices_constant_cop_per_kwh = {int(row["Operating Year #"]): float(row["Price (constant COP/kWh)"]) for _, row in edited.iterrows()}
...
# -----------------------------
with tab_compare:
    st.subheader("Scenario comparison (revenues-only for now)")
    if not compare_scenarios:
            ...
            rows.append(
                {
                    ...
                    "Avg Price (COP/kWh)": float(np.average(dfx["Price (COP/kWh)"], weights=dfx["Energy (MWh)"])),
                    "Total Energy (MWh)": float(dfx["Energy (MWh)"].sum()),
                }
            )
        ...
        st.caption("Next step: CAPEX, OPEX, taxes, debt, and IRR/NPV. Scenario framework is ready.")
